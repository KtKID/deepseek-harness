...
The Host-owned snapshot keeps enabled non-group Loader entries in Loader order and maps root Fiber phases to `running`, `pending`, `starting`, `failed`, `stopping`, or `not-mounted`. Summary cards report enabled entries, running entries, entries carrying diagnoses, and missing declared dependencies. Each row adds Fiber, effect, listener, and missing-dependency counts to its module identity and phase.

Expansion displays the observation origin, normalized effect labels, provided services, declared dependency status and provider identity, direct/transitive impact counts and entry ids, lifecycle transition totals, and the retained transition timeline. Activity is labeled `registrations-only`, so registered listeners remain distinguishable from measured dispatch execution. Disabled entries remain available in the separate [Plugin list](../ui-settings-plugin-inventory/README.md).
