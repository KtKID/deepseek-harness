/**
 * Health checks for DeepSeek Harness plugin bundles and profiles.
 *
 ...
 * @module @deepseek-ai/dsh-plugin-doctor/doctor
 */
...
import { decodeSession, findBrokenToolCalls, type BrokenToolCall } from './session-log.ts'

/** One named diagnostic and its human-readable outcome. */
...
/** Complete bundle inspection result. */
export interface DoctorReport {
  ok: boolean
  checks: CheckResult[]
}

/** One bounded command request issued by an executable doctor check. */
...
/** Settled command outcome normalized for doctor checks. */
export interface DoctorCommandResult {
  code: number
  output: string
}

/** Execution adapter used by build, pack, install, and config checks. */
export type DoctorCommandRunner = (request: DoctorCommand) => Promise<DoctorCommandResult>

/** Optional executable checks, limits, and adapters for one inspection. */
export interface DoctorOptions {
  /** Run build (pnpm run build) when a build script exists. */
  build?: boolean
  /** Full mode: pack, install into a temporary dsh profile, and verify its composed config. */
  full?: boolean
  ...
  maxOutputBytes?: number
  /** Abort all executable checks and cleanup their owned process trees. */
  signal?: AbortSignal | undefined
  /** Command adapter; the model-facing plugin supplies the configured shell executor. */
  runCommand?: DoctorCommandRunner
  /** Run the optional external supply-chain scanner. */
  scanSupplyChain?: boolean
}
 ...
 * @returns the duplicate-runtime-package diagnostic.
 */
 ...
 * @param profileDir - absolute path of the dsh profile to inspect.
 * @returns the profile-manifest encoding diagnostic.
 */
 ...
 * @param profileDir - profile tree to walk.
 * @param thresholdBytes - minimum file size reported as large.
 * @returns the bounded large-file diagnostic.
 */
 ...
 * @param profileDir - absolute path of the dsh profile to inspect.
 * @returns the installed-entry-point diagnostic.
 */
 ...
 * @param profileDir - absolute path of the dsh profile to inspect.
 * @returns the runtime dependency-tree diagnostic.
 */
 ...
 * @param profileDir - absolute path of the dsh profile to inspect.
 * @returns the native-module presence diagnostic.
 */
 ...
 * @returns the durable tool-call pairing diagnostic.
 */
...
const PRE_EXECUTE_RE = new RegExp("(['\"](?:tools/)?pre[_-]?execute['\"]|\\.pre[_-]?execute\\s*\\()", 'i')
...
function collectAuditSourceFiles(root: string): string[] {
  const sourceRoot = path.join(root, 'src')
  return collectSourceFiles(existsSync(sourceRoot) ? sourceRoot : root)
}
 ...
 * @param dir - source tree to scan.
 * @returns the pre-execute side-effect diagnostic.
 */
export function checkPreExecuteSideEffects(dir: string): CheckResult {
  const root = path.resolve(dir)
  const files = collectAuditSourceFiles(root)
  const hits: string[] = []
 ...
 * @param dir - source tree to scan.
 * @returns the shell-launcher diagnostic.
 */
 ...
 * @param timeoutMs - maximum scanner runtime.
 * @param maxOutputBytes - maximum captured scanner output.
 * @returns the external scanner diagnostic or an availability warning.
 */
export function checkSupplyChainSecurity(
  dir: string,
  timeoutMs = 120_000,
  maxOutputBytes = 64_000,
): CheckResult {
  const root = path.resolve(dir)
  const probe = execaSync('dsh-poison-guard', ['scan', root, '--json'], {
    encoding: 'utf8',
...
/** Manifest fields relevant to bundle health checks. */
...
/**
 * Read and summarize the plugin manifest.
 * @param dir - package directory containing `package.json`.
 * @returns selected bundle fields, or `null` when the manifest is absent or invalid.
 */
  ...
  try {
    ...
    return {
      ...
      ...(typeof raw.scripts === 'object' && raw.scripts !== null
        && typeof (raw.scripts as Record<string, unknown>).prepare === 'string'
        ? { prepare: (raw.scripts as Record<string, string>).prepare }
        : {}),
      ...(bundle !== null && typeof bundle === 'object' && typeof bundle.patch === 'string'
        ? { patch: bundle.patch }
        : {}),
...
/** One inserted plugin identity extracted from a bundle patch. */
...
/**
 * Parse a bundle patch and extract inserted plugin ids.
 * @param content - complete `cordis.patch.yml` text.
 * @returns inserted rows carrying an id.
 */
...
async function run(request: DoctorCommand): Promise<DoctorCommandResult> {
  const [file, ...args] = request.argv
  if (file === undefined || file.length === 0) {
    return { code: -1, output: 'doctor command argv must contain a program' }
  }
  ...
  return {
    ...
    output: result.timedOut ? `${output}\n[timed out after ${request.timeoutMs}ms]` : output,
  }
}
 ...
 * @returns the settled ordered diagnostics.
 */
export async function doctor(dir: string, options: DoctorOptions = {}): Promise<DoctorReport> {
  ...
  checks.push(manifest.prepare === undefined
    ? {
      name: 'source-install',
      status: 'WARN',
      detail: 'no prepare script; npm tarballs remain installable, while direct git installs require prebuilt artifacts',
    }
    : {
      name: 'source-install',
      status: 'PASS',
      detail: 'prepare script builds artifacts for direct git installs',
    })
  ...
  checks.push(options.scanSupplyChain === false
    ? {
      name: 'supply-chain-security',
      status: 'WARN',
      detail: 'external supply-chain scanner disabled for this caller; static doctor checks still ran',
    }
    : checkSupplyChainSecurity(root, timeout, maxOutputBytes))
  ...
  if (options.full === true) {
    ...
    const packDir = path.join(home, 'pack')
    mkdirSync(packDir)
    const profile = 'doctor'
    try {
      options.signal?.throwIfAborted()
      const pack = await runCommand({
        argv: ['pnpm', 'pack', '--pack-destination', packDir],
        cwd: root,
        timeoutMs: timeout,
        maxOutputBytes,
        signal: options.signal,
      ...
      if (pack.code !== 0) {
        ...
        if (tarballs.length !== 1) {
          checks.push({
            name: 'pack',
            status: 'FAIL',
            detail: `pnpm pack produced ${tarballs.length} tarballs in its isolated output directory`,
          })
          return { ok: false, checks }
        }
        const tarball = path.join(packDir, tarballs.join(''))
        checks.push({ name: 'pack', status: 'PASS', detail: `packed ${path.basename(tarball)}` })
        const env = { DSH_HOME: home }
        const add = await runCommand({
          argv: [...dsh, 'plugin', '--profile', profile, 'add', tarball],
          cwd: root,
          timeoutMs: timeout,
          maxOutputBytes,
          env,
 ...
 * @param report - settled bundle diagnostics.
