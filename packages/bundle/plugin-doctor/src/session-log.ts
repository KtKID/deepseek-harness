 ...
 * @module @deepseek-ai/dsh-plugin-doctor/session-log
 */
...
/**
 * Decode raw or concatenated-Zstandard session storage records.
 * @param file - session JSONL or JSONL-Zstandard file.
 * @returns the optional header and expanded logical events.
 */
export function decodeSession(file: string): { header: unknown; events: unknown[] } {
  ...
  for (const line of plain.split(/\r?\n/u)) {
    ...
    if (header === undefined && value !== null && typeof value === 'object'
...
/** Declared tool call that has no matching durable result. */
 ...
 * @param events - decoded logical session events.
 * @returns unresolved call identities in declaration order.
 */
export function findBrokenToolCalls(events: unknown[]): BrokenToolCall[] {
  ...
  for (const event of events as Array<Record<string, unknown>>) {
    ...
    if (type === 'assistant/message') {
      ...
      const blocks = message !== null && typeof message === 'object' && Array.isArray((message as Record<string, unknown>).content)
      ...
      const message = data.message
      const content = message !== null && typeof message === 'object' && Array.isArray((message as Record<string, unknown>).content)
        ? (message as { content: Array<Record<string, unknown>> }).content
        : []
      const first = content.at(0)
      const firstContent = first !== undefined && Array.isArray(first.content)
        ? first.content as Array<Record<string, unknown>>
        : []
      const callId = (first?.toolCallId ?? firstContent.at(0)?.toolCallId) as string | undefined
