...
/** Standalone CLI for bundle, profile, and environment checks. */

import { parseArgs } from 'node:util'
import { explainEnvKey, formatEnvExplain } from './env-explain.ts'
import { checkEnvironment, formatEnvReport } from './env.ts'
import {
  checkEntryPoints,
  checkLargeFiles,
  checkManifestBom,
...
Options:
  ...
  --supply-chain  run the optional dsh-poison-guard executable
...
if (values.env) {
  ...
  const checks = await checkEnvironment(Number.isInteger(port) && port > 0 ? port : 3080, Number(values.timeout))
...
if (positionals[0] === 'preflight' || positionals[0] === 'check') {
  ...
  const dir = positionals.at(1) ?? '.'
  const report = await doctor(dir, {
    build: true,
    full: true,
    scanSupplyChain: values['supply-chain'],
