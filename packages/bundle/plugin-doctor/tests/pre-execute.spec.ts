...
describe('checkPreExecuteSideEffects', () => {
  ...
  it('recognizes the harness tools/pre-execute event name', () => {
    const dir = tempDir()
    try {
      writeFileSync(path.join(dir, 'index.ts'), [
        "ctx.on('tools/pre-execute', (_exec, next) => next())",
      ].join('\n'))
      const result = checkPreExecuteSideEffects(dir)
      expect(result.status).toBe('PASS')
      expect(result.detail).toContain('pre-execute listeners present (1 file(s))')
    } finally {
      rmSync(dir, { recursive: true, force: true })
    }
  })
  ...
  it('uses authored source when generated output duplicates bundled dependencies', () => {
    const dir = tempDir()
    try {
      mkdirSync(path.join(dir, 'src'))
      mkdirSync(path.join(dir, 'lib'))
      writeFileSync(path.join(dir, 'src', 'index.ts'), "ctx.on('tools/pre-execute', (_exec, next) => next())")
      writeFileSync(path.join(dir, 'lib', 'index.js'), [
        "import { spawnSync } from 'node:child_process'",
        "ctx.on('tools/pre-execute', () => spawnSync('node', []))",
      ].join('\n'))
      const result = checkPreExecuteSideEffects(dir)
      expect(result.status).toBe('PASS')
      expect(result.detail).toContain('pre-execute listeners present (1 file(s))')
