...
The current Client assembly mounts the application-owned Goal and read-only Host plugin inventory (`pluginInventory/list`) Remote contributions. Cordis effect ownership withdraws every contribution when this assembly unloads, while `@deepseek-ai/dsh-api-gateway/client` owns descriptor validation, traced namespace Services, direct and scoped methods, invocation, and cancellation. Installable Client plugins mount their own generated `/remote` contributions through the same `ctx.remote.$mount()` service, so adding one does not change this application assembly. The Client entry consumes the shared `TypertClientRemote` interface through Cordis and does not import the concrete Gateway. It re-exports the Gateway Client face's declaration merges type-only, so a consumer reaching the forwarded-event vocabulary through this facade gains no runtime edge to the Gateway implementation.
...
- Additional application-owned capabilities require an explicit `/remote` value import and mount here; installable Client plugins own their contribution import and lifetime.
