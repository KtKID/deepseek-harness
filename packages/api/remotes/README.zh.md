...
当前 Client 组合挂载应用自有的 Goal Remote 与只读 Host 插件清单 Remote 贡献（`pluginInventory/list`）。该组合卸载时，Cordis effect 的所有权机制会撤回所有贡献；`@deepseek-ai/dsh-api-gateway/client` 负责描述符校验、可追踪 namespace Service、直接与作用域方法、调用与取消。可安装 Client 插件通过同一 `ctx.remote.$mount()` 服务挂载自己的生成 `/remote` 贡献，因此增加这类插件无需修改本应用组合。Client 入口通过 Cordis 消费共享的 `TypertClientRemote` 接口，不导入具体 Gateway；它只以 type-only 形式重新导出 Gateway Client face 的声明合并，因此消费端经由本外观取到转发事件词汇时，运行时不会多出一条通往 Gateway 实现的边。
...
- 若要增加应用自有能力，必须显式导入相应的 `/remote` 值并在此挂载；可安装 Client 插件自行归属其贡献导入与生命周期。
